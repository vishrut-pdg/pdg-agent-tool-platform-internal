...
export default function Logo({ folded, size, className }: LogoProps) {
  ...
  const logoUrl = settings.enterpriseSettings?.logo_url;
  ...
  // Precedence: env-driven logo_url > admin-uploaded logo > built-in Onyx SVG.
  const logo =
    logoUrl || settings.enterpriseSettings?.use_custom_logo ? (
      <div
        className={cn(
      ...
      >
        ...
        <img
          alt="Logo"
          src={logoUrl ?? `/api/enterprise-settings/logo?v=${logoBuster}`}
            ...
            {!NEXT_PUBLIC_DO_NOT_USE_TOGGLE_OFF_DANSWER_POWERED &&
              !settings.enterpriseSettings?.hide_onyx_branding && (
                <Text
                  ...
                  nowrap
                >
                  Powered by {applicationName || "Onyx"}
                </Text>
              )}
          </div>
        )}
